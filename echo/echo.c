...
{
	char *from = kmalloc(count * sizeof(char ), GFP_KERNEL);

	if(copy_to_user(buff , from, count) != 0 ) 
	{
		return -1;
	}


	printk(KERN_NOTICE "READ FUNC: STRING LOADED: %s\n " , from);
...
ssize_t write(struct file *filep, const char __user *buff, size_t count, loff_t *offp)
{
	...
	if(copy_from_user(from , buff , count) != 0)
	{
		return -1;
	}
	...
	printk(KERN_NOTICE "WRITE FUNC: STRING LOADED: %s\n " , from);
...
static int echo_init(void)
{
	...
	if (!echo_devices) {
		...
		printk(KERN_NOTICE "DEVICES ALLOCATION FAILED\n");
